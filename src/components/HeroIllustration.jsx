...
function JourneyNode({ index, emoji, label, size, ring, className }) {
  return (
    <motion.div
    ...
    >
      <motion.div
        ...
        className={`flex items-center justify-center rounded-full bg-cream-50 shadow-md ring-4 ${ring} ${size}`}
      >
        {emoji}
      </motion.div>
  ...
  return (
    <div className="relative mx-auto h-72 w-72 sm:h-80 sm:w-80 md:h-96 md:w-96">
      ...
      <JourneyNode
        ...
        emoji="🍱"
        label="Posted"
        size="h-20 w-20 text-4xl sm:h-24 sm:w-24 sm:text-5xl"
      ...
      <JourneyNode
        ...
        emoji="🛵"
        label="On the way"
        size="h-14 w-14 text-2xl sm:h-16 sm:w-16 sm:text-3xl"
      ...
      <motion.span
        className="absolute bottom-16 left-0 text-sm text-olive-300 sm:bottom-20"
        aria-hidden="true"
        animate={{ opacity: [0.3, 1, 0.3], scale: [0.9, 1.15, 0.9] }}
